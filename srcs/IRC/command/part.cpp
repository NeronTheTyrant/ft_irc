...
void	IRCServer::part(User *sender, std::vector<std::string> params) {
	...
	if (params.size < 2) {
		sender.send(serverMessageBuilder(*this, commandMessageBuilder(CODE_ERR_NEEDMOREPARAMS, "PART")));
		return ;
	}
	if (target == u_nullptr) {
		sender.send(serverMessageBuilder(*this, commandMessageBuilder(CODE_ERR_NOSUCHCHANNEL, params[1])));
		return ;
	}
	else if (target->users.find(sender) == target->users.end()) {
		sender.send(serverMessageBuilder(*this, commandMessageBuilder(CODE_ERR_NOTONCHANNEL, params[1])));
		return ;
	}
